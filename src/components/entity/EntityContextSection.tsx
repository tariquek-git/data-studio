  ...
  return (
    <TerminalCard title={section.title} subtitle={section.summary}>
      <div className={`mb-4 rounded-2xl border p-4 ${theme.banner}`}>
        <p className="text-sm leading-relaxed text-slate-900">{theme.copy}</p>
      </div>
              ...
              >
                <div className="flex items-start justify-between gap-3">
                  <div className="min-w-0">
                    ...
                    <p className="mt-2 text-sm font-medium leading-relaxed text-slate-900">{item.value}</p>
                  </div>
                  {item.url && (
                    <a href={item.url} target="_blank" rel="noopener noreferrer" className="text-slate-700 hover:text-slate-900">
