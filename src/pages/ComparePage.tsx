    ...
    <div ref={wrapperRef} className="relative w-full max-w-sm">
      <div className="relative">
        ...
        <input
          ...
          disabled={disabled}
          className="block w-full rounded-lg border border-surface-300 bg-white pl-9 pr-3.5 py-2 text-sm text-surface-900 placeholder:text-surface-400 focus:border-primary-500 focus:ring-1 focus:ring-primary-500 focus:outline-none disabled:opacity-50"
        />
        {isFetching && (
      ...
      {open && query.trim().length >= 2 && (
        <div className="absolute z-50 mt-1 w-full bg-white border border-surface-200 rounded-xl shadow-lg overflow-hidden">
          {results.length === 0 && !isFetching ? (
            ...
            <ul className="max-h-64 overflow-auto divide-y divide-surface-100">
              {results.map(inst => {
                ...
                return (
                  <li key={cert}>
                    <button
                      ...
                      disabled={already}
                      className="w-full text-left px-4 py-2.5 hover:bg-surface-50 transition-colors disabled:opacity-40 disabled:cursor-not-allowed"
                    >
                      <p className="text-sm font-medium text-surface-900 truncate">{inst.name}</p>
...
function ColumnHeader({ cert, onRemove }: ColumnHeaderProps) {
  ...
  return (
    <div className="min-w-[180px] p-3 relative group">
      <button
        onClick={onRemove}
        className="absolute top-2 right-2 p-1 rounded-md text-surface-400 hover:text-red-500 hover:bg-red-50 transition-colors opacity-0 group-hover:opacity-100"
      ...
      >
        <span className="text-sm font-semibold text-surface-900 leading-snug hover:text-primary-600 transition-colors pr-5">
          {inst.name}
        </span>
        <ArrowUpRight className="h-3.5 w-3.5 text-surface-400 group-hover/link:text-primary-500 shrink-0 mt-0.5 transition-colors" />
      </Link>
...
function DataCell({ cert, metric, highlight }: DataCellProps) {
  ...
  if (!data) {
    return (
      <td className="px-3 py-2.5 border-b border-surface-100 text-sm text-surface-400">—</td>
    );
  }
  ...
  return (
    <td className={`px-3 py-2.5 border-b border-surface-100 text-sm whitespace-nowrap ${hlClass}`}>
      {display}
    </td>
  );
}
...
export default function ComparePage() {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-6">
      {/* Page header */}
      <div>
        <h1 className="text-2xl font-bold text-surface-900">Compare Institutions</h1>
      ...
      <div className="flex items-center gap-3 flex-wrap">
        ...
        />
        {atMax && (
          <p className="text-xs text-surface-400">Maximum 5 institutions reached.</p>
        )}
        {certList.length > 0 && (
          <button
            onClick={() => setCerts([])}
          ...
          <table className="w-full border-collapse bg-white">
            <thead>
              <tr className="border-b border-surface-200 bg-surface-50">
                {/* Metric label column */}
                <th className="w-44 px-4 py-3 text-left text-xs font-semibold text-surface-500 uppercase tracking-wide border-r border-surface-200">
                  Metric
                </th>
                {certList.map(cert => (
                  <th key={cert} className="border-r border-surface-100 last:border-r-0">
            ...
            <tbody>
              {METRICS.map((metric, mi) => {
                ...
                return (
                  <tr
                    key={metric.label}
                    className={isEven ? 'bg-white' : 'bg-surface-50/50'}
                  >
                    <td className="px-4 py-2.5 text-xs font-medium text-surface-600 border-b border-surface-100 border-r border-surface-200 whitespace-nowrap">
                      {metric.label}
                    </td>
                    {certList.map(cert => (
                      <DataCell
